...
const username = 'sharp0111'; // TODO: Replace with your username
...
server.get('/secretgist/:id', (req, res) => {
  ...
  github.gists.get({ id })
  .then((result) => {
    const { content } = Object.values(result.data.files)[0];
    // console.log(content);
    const nonce = nacl.util.decodeBase64(content.slice(0, 32));
    const encryptContent = nacl.util.decodeBase64(content.slice(32, content.length));
    const decryptContent = nacl.secretbox.open(encryptedContent, nonce, key);
...
server.post('/createsecret', urlencodedParser, (req, res) => {
  ...
  const { name, content } = req.body;
  // let secret_key = nacl.randomBytes(32);
...
server.post('/postmessageforfriend', urlencodedParser, (req, res) => {
  ...
  if (savedKey === undefined) {
    ...
    const nonce = nacl.randomBytes(24);
    const ciphertext = nacl.box(nacl.util.decodeUTF8(content), nonce, nacl.util.decodeBase64(publicKey), key);
    const blob = nacl.util.encodeBase64(nonce) + nacl.util.encodeBase64(ciphertext);
    const files = { [name]: { content: blob } };
    ...
    github.gists.create({ files, public: true })
      .then((response) => {
        ...
        const messageString = nacl.util.encodeBase64(keypair.publicKey) + id;
...
server.get('/fetchmessagefromfriend:messageString', urlencodedParser, (req, res) => {
  ...
  const friendKey = nacl.util.decodeBase64(messageString.slice(0, 44));
  ...
  .then((response) => {
    ...
    const decryptedContent = nacl.box.open(encryptedContent, nonce, friendKey, key);
    res.send({ message: nacl.util.encodeUTF8(decryptedContent) });
  }).catch((err) => {
    res.send(err);
  });
});
