...
const username = 'RiaClay'; // TODO: Replace with your username
...
const key = process.env.SECRET_KEY ? nacl.decodeBase64(process.env.SECRET_KEY) : nacl.randomBytes(32);
...
server.get('/fetchmessagefromself:id', (req, res) => {
  ...
  const gist = github.gists.get({ id }).then(result => { res.json(result)});
});
...
server.post('/createsecret', urlencodedParser, (req, res) => {
  ...
  const encryption = nacl.secretbox(nacl.util.decodeUTF8(content), nonce, secretKey);

  content = nacl.util.encodeBase64(encryption);
