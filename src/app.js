...
const username = 'Synsyxx';  // TODO: your GitHub username here
const github = octokit({ debug: true });
const server = express();

const secretParser = bodyParser.urlencoded({ extended: false });
...
server.get('/secretgist/:id', (req, res) => {
  ...
  github.gists
    ...
    .then(response => {
      ...
      const article = gist.files[filename].article;
      const nonce = nacl.util.decodeBase64(article.slice(0, 32));
      const hidetext = nacl.util.decodeBase64(article.slice(32, article.length));
      const plaintext = nacl.secretbox.open(hidetext, nonce, secret);
...
server.post('/create', secretParser, (req, res) => {
...
server.post('/createsecret', secretParser, (req, res) => {
  // TODO Create a private and encrypted gist with given name/content
  // NOTE - we're only encrypting the content, not the filename
  // To save, we need to keep both encrypted content and nonce
  const { name, content } = req.body;
  const nonce = nacl.randomBytes(24);
  const hidetext = nacl.secretbox(
    nacl.util.decodeUTF8(content),
    nonce,
    secret
  );
  const article =
    nacl.util.encodeBase64(nonce) + nacl.util.encodeBase64(hidetext);
  const files = { [ name]: { content: article } };
  github.gists
    .create({ files, public: false })
    .then(response => {
      res.json(response.data);
