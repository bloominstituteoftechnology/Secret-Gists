...
const username = 'Beeness'; // TODO: Replace with your username
...
server.get('/fetchmessagefromself:id', (req, res) => {
  ...
  github.gists.get({ id })
    .then((response) => {
      ...
      // assume that the gist only contains one file
      const filename = Object.keys(gist.files)[0];
      // grab the encrypted content and nonce
      const blob = gist.files[filename].content;
      // Nonce is the first 24 bytes bytes; splice that many bytes off the blob
      // 24 byte nonce translats to 32 characters once we encode in base 64
      const nonce = nacl.util.decodedBase64(blob.slice(0, 32));
      // Grab the ciphertext from the rest of the blob
      const ciphertext = nacl.util.decodeBase64(blob.slice(32, blob.length));
      // Decrypt the ciphertext into plantext
      const plaintext = nacl.secretbox.open(ciphertext, nonce, secretKey);
      // Send the plaintest in the response
