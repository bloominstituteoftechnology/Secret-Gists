...
let secretKey = process.env.SECRET_KEY ?
  nacl.util.decodeBase64(nacl.util.encodeBase64(process.env.SECRET_KEY))
  : nacl.randomBytes(32);
...
server.get('/setkey:keyString', (req, res) => {
  ...
  try {
    secretKey = nacl.util.decodeBase64(nacl.util.encodeBase64(keyString));
  } catch (err) {
    // failed
...
server.get('/fetchmessagefromself:id', (req, res) => {
  ...
  github.gists.get({ id })
    .then((response) => {
      ...
      const encryptedContent = response.data.files[name].content;
      const nonce = nacl.util.decodeBase64(encryptedContent.slice(0, 32));
      const ciphertext = nacl.util.decodeBase64(encryptedContent.slice(32));
      const msg = nacl.secretbox.open(ciphertext, nonce, secretKey);

      res.send(nacl.util.encodeUTF8(msg));
    })
    .catch((err) => {
      res.json(err);
    });
...
server.post('/createsecret', urlencodedParser, (req, res) => {
  ...
  let encryptedContent = nacl.util.encodeBase64(nonce) + nacl.util.encodeBase64(ciphertext);
